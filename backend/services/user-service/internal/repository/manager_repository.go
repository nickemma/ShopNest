...
func (mgr *PostgresManagerRepository) RegisterManager(ctx context.Context, manager *domain.RegisterManager, auth *domain.Auth) error {
	...
	mgrQuery := `
		INSERT INTO managers (manager_id, name, email, role)
		VALUES ($1, $2, $3, $4)`

	_, err = tx.Exec(ctx, mgrQuery, manager.ManagerID, manager.Name, manager.Email, manager.Role)
	if err != nil {
		return err
	}
...
func (mgr *PostgresManagerRepository) GetManagerByEmail(ctx context.Context, email string) (*domain.Manager, error) {
	...
	// Query manager
	managerQuery := `SELECT manager_id, name, email, role, created_at, updated_at
